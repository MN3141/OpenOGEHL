...
propriul build target CMake: aplicatia principala si suita de unit
tests.

\subsubsection*{Aplicatia principala}

Aplicatia este scrisa in C si foloseste doua librarii externe pentru
interfata grafica:
...
\begin{itemize}
    \item \textbf{raylib} --- librarie grafica multiplatforma scrisa in C,
          ...
          linkuita static din \texttt{libs/raylib/lib/libraylib.a}.
    \item \textbf{raygui} --- librarie de UI imediat (\textit{immediate
          mode GUI}) construita peste raylib, folosita pentru a crea
          toate controalele interfetei (campuri de text, butoane, liste).
          Este inclusa ca fisier header-only direct in sursa.
\end{itemize}

Alegerea acestor librarii a fost motivata de faptul ca libraria
...
librarii externe. Fisierele sursa ale modulului \texttt{bp/} au fost
