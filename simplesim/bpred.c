    ...
    switch (class)
    {
        ...
        case BPredOGHEL:
        {
            ...
            if ((retstack_size & (retstack_size - 1)) != 0)
                fatal(
                    "Return-address-stack size must be zero or a power of two");

            pred->retstack.size = retstack_size;
            if (retstack_size)
                if (!(pred->retstack.stack = calloc(
                          retstack_size,
                          sizeof(struct bpred_btb_ent_t))))
    ...
    switch (class)
    {
        case BPred2Level:
        {
            if (!l1size || (l1size & (l1size - 1)) != 0)
                fatal(
                    "level-1 size, `%d', must be non-zero and a power of two",
                    l1size);
            pred_dir->config.two.l1size = l1size;

            if (!l2size || (l2size & (l2size - 1)) != 0)
                fatal(
                    "level-2 size, `%d', must be non-zero and a power of two",
                    l2size);
            pred_dir->config.two.l2size = l2size;

            if (!shift_width || shift_width > 30)
                fatal(
                    "shift register width, `%d', must be non-zero and positive",
                    shift_width);
        ...
        case BPred2bit:
            if (!l1size || (l1size & (l1size - 1)) != 0)
                fatal(
                    "2bit table size, `%d', must be non-zero and a power of "
                    "two",
                    l1size);
    ...
    FILE *stream)         /* output stream */
{
    switch (pred->class)
    {
        ...
        case BPredOGHEL:
            fprintf(
                stream,
                "pred: OGHEL O-GEHL neural predictor: "
    ...
    md_addr_t baddr)              /* branch address */
{
    ...
    switch (pred_dir->class)
    {
        case BPred2Level:
        {
            ...
            l1index =
                (baddr >> MD_BR_SHIFT) & (pred_dir->config.two.l1size - 1);
					 ...
					 * used on mispredict recovery */
{
    ...
    switch (pred->class)
    {
        ...
        case BPredOGHEL:
            if ((MD_OP_FLAGS(op) & (F_CTRL | F_UNCOND)) != (F_CTRL | F_UNCOND))
            {
                ...
                bool oghel_taken = BP_GetPrediction(
                    (uint32_t)baddr,
   ...
   */
    ...
    if (pbtb == NULL)
    {
        ...
        return (
            (*(dir_update_ptr->pdir1) >= 2) ? /* taken */ 1
                                            : /* not taken */ 0);
    }
    else
    {
        ...
        return (
            (*(dir_update_ptr->pdir1) >= 2) ? /* taken */ pbtb->target
                                            : /* not taken */ 0);
    }
}
    ...
    if (taken)
    {
	   ...
	   * MRU and LRU items */
            for (i = index; i < (index + pred->btb.assoc); i++)
            {
                ...
                dassert(
                    pred->btb.btb_data[i].prev != pred->btb.btb_data[i].next);
