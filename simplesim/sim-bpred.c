...
static int twolev_config[4] = {/* l1size */ 1,
                               /* l2size */ 1024,
                               /* hist */ 8,
                               /* xor */ FALSE};
...
static int oghel_config[3] = {/* num_tables */ 7,
                              /* table_size */ 2048,
                              /* counter_len */ 4};
...
void sim_check_options(struct opt_odb_t *odb, int argc, char **argv)
{
    ...
    else if (!mystricmp(pred_type, "2lev"))
    {
        ...
        if (twolev_nelt != 4)
            fatal(
                "bad 2-level pred config (<l1size> <l2size> <hist_size> "
                "<xor>)");
        if (btb_nelt != 2)
            fatal("bad btb config (<num_sets> <associativity>)");
    ...
    else if (!mystricmp(pred_type, "oghel"))
    {
        ...
        if (oghel_nelt != 3)
            fatal(
                "bad oghel predictor config (<num_tables> <table_size> "
                "<counter_len>)");
        if (btb_nelt != 2)
            fatal("bad btb config (<num_sets> <associativity>)");
